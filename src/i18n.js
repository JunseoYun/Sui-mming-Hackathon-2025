const translations = {
  ko: {
    ...
    'system.adventureSummary': '모험 완료! 전투 {{battles}}회, 포획 {{captured}}마리, 보상 +{{tokens}} BM',
    'system.fusionCreated': '새로운 블록몬이 탄생했습니다! 인벤토리에서 확인하세요.',
    ...
    'home.subtitle': '랜덤 DNA로 탄생한 블록몬 팀을 성장시키고 SUI 바다를 정복하세요.',
    'home.section.blockmons': '내 블록몬',
    'home.noBlockmons': '아직 보유한 블록몬이 없습니다. DNA를 합성하거나 모험에서 확보하세요.',
    'home.selectedCount': '선택된 팀: {{count}} / 4',
    'home.clearSelection': '선택 초기화',
    'home.selection.summaryLead': '선택된 블록몬 {{count}}마리',
    'home.selection.extraNotice': '※ 합성 시 가장 먼저 선택한 두 마리가 사용됩니다.',
    ...
    'home.activeAdventure.summary': '{{active}} / {{total}} 명 활동 중 · 포션 {{potions}}개 · 총 사용 토큰 {{spent}} · 포획 {{captured}}마리',
    ...
    'adventure.subtitle': '모험을 시작하면 팀이 모두 탈진할 때까지 야생 블록몬과 연속 전투를 수행합니다.',
    ...
    'battleLog.entry.player': '[내 블록몬 - {{name}}] {{action}} ({{detail}}) | 야생 {{target}} HP {{hp}}',
    'battleLog.entry.opponent': '[야생 블록몬 - {{name}}] {{action}} ({{detail}}) | 내 {{target}} HP {{hp}}',
    ...
    'fusion.title': 'DNA 합성',
    'fusion.subtitle': '같은 종족의 DNA를 조합해 새로운 블록몬을 탄생시키세요. (비용 1 BM)',
    'fusion.section.blockmons': '보유 블록몬',
    'fusion.tokens': '현재 토큰: {{value}} BM',
    'fusion.selection.lead': '선택된 블록몬 {{count}}마리 (같은 종족만 유지됩니다.)',
    'fusion.selection.notice': '※ 합성 시 가장 먼저 선택한 두 마리가 사용됩니다.',
    ...
    'fusion.result.message': '{{name}}이(가) 합성되었습니다! 인벤토리에서 확인하세요.',
    'fusion.alert.title': '{{name}} 탄생!',
    'fusion.alert.dna': 'DNA: {{dna}}',
    'pvp.title': 'PVP 매칭',
    'pvp.subtitle': '3 BM 토큰을 배팅하여 다른 수호자와 전투하세요. 승리 시 5 BM 획득, 1 BM 수수료 차감.',
    ...
    'inventory.adventure.summary': '상태: {{status}} · 팀 {{team}} / 탈진 {{defeats}} · 포획 {{captured}}마리 · 포션 {{potions}} · 사용 토큰 {{spent}}',
    'inventory.adventure.button': '모험 로그 보기',
    'inventory.status.active': '진행 중',
    'inventory.status.complete': '완료',
    'signup.title': 'BLOCKMON BATTLE',
    'signup.subtitle': '당신만의 랜덤 시드를 받아 첫 번째 블록몬 DNA를 생성하세요.',
    'signup.description': '랜덤 시드로 생성된 DNA는 고유성을 보장하며, 첫 블록몬과 함께 10개의 BM 토큰을 지급합니다.\n모험과 배틀을 통해 토큰과 새로운 DNA를 수집하고 인벤토리를 확장하세요.',
    ...
    'signup.bullet.adventure': '✅ 모험 시작 시 토큰 1개 소모, 배틀 승리 시 추가 보상',
    'signup.bullet.fusion': '✅ 동일 종족 2마리 합성 → 새로운 DNA + 능력치 강화'
  },
  en: {
    'app.brand': 'Blockmon Battle Prototype',
    'system.starterCreated': 'Starter Blockmon DNA created!',
    'system.adventureSummary': 'Adventure complete! Battles {{battles}}, captured {{captured}}, reward +{{tokens}} BM',
    'system.fusionCreated': 'A new Blockmon has been born! Check your inventory.',
    ...
    'home.subtitle': 'Grow your Blockmon squad and conquer the SUI seas with unique DNA.',
    'home.section.blockmons': 'My Blockmon',
    'home.noBlockmons': 'No Blockmon yet. Fuse DNA or capture them during adventures.',
    ...
    'home.selection.extraNotice': '※ The first two selected Blockmon will be used for fusion.',
    ...
    'home.activeAdventure.summary': '{{active}} / {{total}} active · Potions {{potions}} · Tokens spent {{spent}} · Captured {{captured}}',
    ...
    'adventure.prepSubtitle': 'Start an adventure from the home screen to view its log here.',
    'adventure.none': 'No adventure is running right now.',
    'adventure.heading': 'Adventure Mission',
    'adventure.subtitle': 'Once started, the team battles wild Blockmon endlessly until everyone faints.',
    ...
    'battleLog.entry.player': '[My Blockmon - {{name}}] {{action}} ({{detail}}) | Wild {{target}} HP {{hp}}',
    'battleLog.entry.opponent': '[Wild Blockmon - {{name}}] {{action}} ({{detail}}) | My {{target}} HP {{hp}}',
    ...
    'fusion.subtitle': 'Combine DNA from the same species to create a new Blockmon. (Cost 1 BM)',
    'fusion.section.blockmons': 'Owned Blockmon',
    'fusion.tokens': 'Current tokens: {{value}} BM',
    'fusion.selection.lead': '{{count}} Blockmon selected (same species required).',
    'fusion.selection.notice': '※ The first two selected Blockmon will be fused.',
    ...
    'pvp.subtitle': 'Stake 3 BM tokens and battle another guardian. Win 5 BM with a 1 BM fee.',
    ...
    'inventory.adventure.summary': 'Status: {{status}} · Team {{team}} / KO {{defeats}} · Captured {{captured}} · Potions {{potions}} · Tokens spent {{spent}}',
    'inventory.adventure.button': 'View Adventure Log',
    'inventory.status.active': 'Active',
    'inventory.status.complete': 'Complete',
    'signup.title': 'BLOCKMON BATTLE',
    'signup.subtitle': 'Claim your random seed and forge the first Blockmon DNA.',
    'signup.description': 'Random seeds grant uniqueness and award your first Blockmon with 10 BM tokens.\nGrow your inventory by adventuring and fusing new DNA.',
    ...
    'signup.bullet.adventure': '✅ Adventures cost 1 BM, battle wins grant extra rewards',
...
const temperamentTranslations = {
  '창공을 가르는 검은 거인': { ko: '창공을 가르는 검은 거인', en: 'Black giant that cleaves the sky' },
  '심연의 빛을 머금은 미립자': { ko: '심연의 빛을 머금은 미립자', en: 'Microbe infused with abyssal light' },
  '백사장 위의 장수 감시자': { ko: '백사장 위의 장수 감시자', en: 'Long-lived guardian watching the shores' },
  '폭풍을 부르는 촉수의 제왕': { ko: '폭풍을 부르는 촉수의 제왕', en: 'Tentacled monarch that summons storms' },
  '바다를 가르는 고대의 용': { ko: '바다를 가르는 고대의 용', en: 'Ancient dragon cleaving the seas' },
  '파도에 노래를 실어 보내는 수호자': { ko: '파도에 노래를 실어 보내는 수호자', en: 'Guardian whose songs ride the waves' },
  '합성으로 각성한 유전자': { ko: '합성으로 각성한 유전자', en: 'Genes awakened through fusion' }
}

const originTranslations = {
  '야생 탄생': { ko: '야생 탄생', en: 'Born in the wild' },
  '스타터 DNA': { ko: '스타터 DNA', en: 'Starter DNA' },
  '포획된 야생 블록몬': { ko: '포획된 야생 블록몬', en: 'Captured wild Blockmon' },
