...
function determineRarity(totalStats) {
  return RARITY_BRACKETS.find((tier) => totalStats >= tier.threshold)?.label ?? 'Common'
}
...
export function fuseBlockmons(parents, seed) {
  ...
  const baseSpecies = speciesCatalog.find((entry) => entry.name === first.species) ?? pickSpecies(seed)
  const fusionSeed = seed ^ BigInt(parseInt(first.seed.slice(0, 8), 16)) ^ BigInt(parseInt(second.seed.slice(8, 16), 16))
  ...
  statKeys.forEach((key, index) => {
    ...
    const adjustment = Math.floor(rng() * (4 + index)) - Math.floor(rng() * (2 + index))
...
export function rollBattleOutcome(player, opponent, seed) {
  const rng = lcg(seed)
  const initiativeScore = player.stats.dex - opponent.stats.dex + Math.floor(rng() * 6) - 3
  ...
  const attackPhase = (attacker, defender, defenderHp) => {
    ...
    const totalDamage = Math.max(4, Math.round(baseDamage + variance)) * (isCrit ? 2 : 1)
  ...
  while (playerHp > 0 && opponentHp > 0 && rounds.length < 8) {
    if (attacker === 'player') {
      ...
      rounds.push({
        ...
        action: phase.result === 'miss' ? '공격이 빗나갔습니다.' : phase.result === 'crit' ? '치명타 성공!' : '공격 성공',
